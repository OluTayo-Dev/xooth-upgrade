...
function DualForm() {
  ...
  return (
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 max-w-6xl mx-auto mt-7 ml-6 xl:lg:ml-[8rem]">
     {ForumPages.map((cat) => (
    <Link
      to={`/${cat.name.toLowerCase()}`}
      key={cat.name}
      className="bg-white w-[8rem] rounded-xl shadow-md flex flex-col items-center p-4 hover:scale-105 transition-transform cursor-pointer"
    >
      <img
        src={cat.img}
        alt={cat.name}
