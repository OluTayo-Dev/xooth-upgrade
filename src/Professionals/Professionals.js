...
function Professionals() {
  ...
  const submitProfession = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      const res = await axios.post("https://xooth-backend.onrender.com/api/professionals", formData, config);
  ...
  useEffect(() => {
    axios
      .get('https://xooth-backend.onrender.com/api/professions')
      .then((res) => {
  ...
  const categories = ['Lawyers', 'Doctors', 'Engineers', 'Pharmacist', 'Teachers', 'Nurses', 'Plumber', 'Bricklayer', 'Others'];
    ...
    <div className="">
           ...
           <div className="bio 2xl:w-1/2 w-[100%] ml-[24px] p-0">
              <p className="text-sm text-gray-800 font-semibold">
                <span className="text-green-600 font-bold">Name:</span>{" "}
                   {item.fullName || "Unnamed Item"}
            
             </p>
                 <p className="text-sm text-gray-800 font-semibold">
                   <span className="text-green-600 font-bold">Profession:</span>{" "}
                ...
                <p className="text-sm text-gray-800 font-semibold">
                 <span className="text-green-600 font-bold">Qualification:</span>{" "}
                         {item.qualification || "Unknown Gender"}
               </p>
      ...
      >
        <div className="rounded-[30px]">
          <form onSubmit={submitProfession} className="form-class xl:w-[80%] w-[100%] h-[50vh] rounded-none">
            <label>Name</label> <br />
            <input type="text" 
            placeholder=""
            id="fullName"
            ...
            <label>Certification/Qualification</label> <br />
            <input type="text" 
            placeholder=""
            id="qualification"  
            onChange={onChangeHandler}
